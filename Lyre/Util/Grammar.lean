...
@[inline] def throwWithSyntax [Monad m] [MonadError m] (stx : Syntax) (caption : MessageData) (ref := stx) : m α :=
  throwErrorAt ref m!"{caption}:{indentD stx}"

@[inline] def throwIllFormedSyntax [Monad m] [MonadError m] (stx : Syntax) (kind := "syntax") : m α :=
  throwWithSyntax stx m!"ill-formed {kind}"
...
| `($[$doc?]? syntax $id : $cats,* := $[$xs]*) => do
  ...
  cats.getElems.forM fun cat => do
    let `(catWithPrio|$catId $[: $prio?]?) := cat
      | throwIllFormedSyntax cat "category"
